...
upstream_cache_clean = t.zeros(
    (N_TOTAL_UPSTREAM_NODES, BATCH_SIZE, SEQUENCE_LENGTH, model.cfg.d_model)
)
...
idx = 0
for stage_cnt, name in enumerate(
    clean_cache.keys()
):  # stage_cnt relevant for keeping track which upstream-downstream mairs can be connected
...
downstream_grad_cache_clean = t.zeros(
    (N_TOTAL_DOWNSTREAM_NODES, BATCH_SIZE, SEQUENCE_LENGTH, model.cfg.d_model)
)
...
for name in names:
    if name.endswith(
        "hook_q_input"
    ):  # do all q k v hooks of that layer simultaneously, as it is the same stage
        q_name = name
...
upstream_level_matrix = einops.repeat(
    upstream_levels,
    "up_nodes -> up_nodes down_nodes",
    down_nodes=N_TOTAL_DOWNSTREAM_NODES,
)
downstream_level_matrix = einops.repeat(
    downstream_levels,
    "down_nodes -> up_nodes down_nodes",
    up_nodes=N_TOTAL_UPSTREAM_NODES,
)
...
def steering_hook_attn(
    attn_pattern: Float[Tensor, "batch seq head_idx d_model"], hook: str
):
    """
    ...
    for [up_name, up_idx], [down_name, down_idx] in topk_dict[hook.name]:
        ...
        # get steering vector
        steering_vector = retrieve_steering_vector(
            clean_cache, corrupted_cache, up_name, up_idx
        )
...
def steering_hook_mlp_resid(
    attn_pattern: Float[Tensor, "batch seq d_model"], hook: str
):
    for [up_name, up_idx], [down_name, down_idx] in topk_dict[hook.name]:
...
def retrieve_steering_vector(clean_cache, corr_cache, up_hook_name, up_hook_index):
    ...
    clean_steering_vector = einops.reduce(
        clean_cache_vec, "batch seq d_model -> seq d_model", "mean"
    )
    corr_steering_vector = einops.reduce(
        corr_cache_vec, "batch seq d_model -> seq d_model", "mean"
    )
